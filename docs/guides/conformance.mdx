...
The first four cover the whole chain from a payload to a version identifier. Agree on all of them and two
clients share a brain; disagree on any and they do not, whatever else they implement. The last two cover
the authenticity role — divergence there is not silent identity drift but a verifier accepting what
another refuses, which is worse.
