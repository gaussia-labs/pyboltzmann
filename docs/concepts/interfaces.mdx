...
| Contract | Operations |
| --- | --- |
| `BrainReader` | `snapshot`, `root_of`, `module`, `open_index`, `resolve`, `prove`, `resolvability`, `verify`, `search`, `browse`, `catalog_path` |
| `BrainWriter` | `register`, `replace`, `define_task`, `validate`, `commit`, `classify` |
...
| `BrainAuthenticity` | `sign`, `authenticate`, `pin`, `plan_rotate`, `countersign`, `rotate`, `revoke`, `signatures`, `add_signature` |
| `BoltzmannProtocol` | all six contracts |
...
Every error derives from `BoltzmannError`, in six families:
