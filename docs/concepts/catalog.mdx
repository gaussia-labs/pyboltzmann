...
The SDK rebuilds the convenient catalog view in memory from those blocks. The view itself is not stored and
does not travel as an OCI layer. This keeps the canonical source unchanged: it is the placement block, not
the canonical block, that carries the classification.

## Declare the taxonomy

Taxonomy design is explicit SDK work. A model may propose placements, but cannot create schemes, classes,
or hierarchy edges.
...
Hierarchies are acyclic and stay inside one scheme. A class may have multiple parents. Browsing a parent
includes sources placed directly in any descendant.
...
internal segments and `.` or `..` are rejected.
