...
Every catalog block is attributed. A scheme, a class or a hierarchy edge cites no evidence, so nothing can
derive it; it receives a registration record instead, whose `origin` names the declaration
